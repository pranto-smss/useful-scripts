<div align="center">
  <h1>🔧 useful-scripts</h1>
  <p><strong>Plug-and-play micro-scripts for everyday automation.</strong></p>
  <p>One file. One job. Zero setup.</p>
  <p>
    <a href="#script-directory">Scripts</a> •
    <a href="#quick-start">Quick Start</a> •
    <a href="#contributing">Contributing</a>
  </p>
  <br>
</div>

---

## Script Directory

| Script | Language | Description | Usage |
|--------|----------|-------------|-------|
| [Facebook Auto Friend Request](javascript/facebook-auto-friend-request/facebook-auto-friend-request.js) | JavaScript (Bookmarklet) | Growth-hack your network on autopilot. Clicks every "Add Friend" button with randomized 4–9s human-mimic delays — hard to detect, easy to scale. Turn a tab into a lead-gen machine. | Paste into browser console on `facebook.com` |
| [Media Compressor](python/media_compressor/media_compressor.py) | Python 3 | Ship that clip. Crush any video down to a target size in one drag-and-drop — Discord's 25 MB cap? Twitter upload limits? Highlight reel too chonky? This thing slims it down with surgical 2-pass precision. | `python media_compressor.py` |
| [Dynamic Media Strip](javascript/dynamic-media-strip/dynamic-media-strip.js) | JavaScript (Console Snippet) | ⚡ **Scroll, Spot, Save.** Auto-harvests every pristine high-res image on any page you scroll — Pinterest, Instagram, news sites, portfolios, you name it. No extensions, no logins, no alt-tabbing. A slick live gallery strip docks to your sidebar with hover-to-zoom previews and one-click saves. Infinite scroll? It watches your back. | Paste into console on any site with images |

---

## Quick Start

1. **Clone the repo**
   ```bash
   git clone https://github.com/<your-org>/useful-scripts.git
   cd useful-scripts
   ```

2. **Pick a script** from the table above and open its file.

3. **Run it** — most scripts are self-contained. Language-specific instructions (e.g. `python script.py`, `node script.js`, `bash script.sh`) are noted in each script's header.

---

## Contributing

We'd love your help! See [CONTRIBUTING.md](CONTRIBUTING.md) for:
- How to fork, branch, and open a PR
- Script template and style guide
- Checklist for a "plug-and-play" submission

---

<div align="center">
  <sub>Built with ☕ by contributors like you.</sub>
</div>
