...
| [Batch Rename](powershell/batch-rename/batch-rename.ps1) | PowerShell | **Just run it.** Picks a folder, filter, and rename operation through clean on-screen prompts — find/replace, prefix, suffix, numbering, case change, or combine them all. Shows a preview before touching anything. Undoes previous renames automatically. | `./batch-rename.ps1` or `batch-rename.bat` |
| [Temp Cleaner](powershell/temp-cleaner/temp-cleaner.ps1) | PowerShell | Clean Windows temp files, caches, and crash dumps. Preview before deleting, age-based filter to keep recent files, skips locked files. Supports `-Mode` for non-interactive use. Levels 2-3 auto-elevate via UAC. | `./temp-cleaner.ps1` or `temp-cleaner.bat` |
...
| [Song Metadata Editor](python/song_metadata_editor/song_metadata_editor.py) | Python 3 | Edit audio file metadata (title, artist, album, year, genre, ISRC) using MusicBrainz. Single file or batch mode. Fetches album art from Cover Art Archive. Renames files to "Artist - Title.ext". | `song_metadata_editor.py` |
